...
void main(void){
  float qu = time+volume*0.1;
  float tile_size = time*0.003*sin(1.*time+0.4)+2.0;
  //tile_size = abs(4.0*cos(time*1.));
  float thick_base = 1.2;
  vec3 fg = vec3(0.1);
  ...
  gl_FragColor = vec4(vec3(color.x,color.y*sin(time)/1.0,color.z*tan(time))*1.0/2.0,1.0);
}
